...
* 체크 예외
  ...
  * 자 식 클래스가 더 넓은 범위의 예외를 던지면 해당 코드는 모든 예외를 제대로 처리하지 못할 수 있으며 이는 예외 처리의 일관성을 해치고, 예상하지 못한 런타임 오류를 초래할 수 있다.
...
#### join
* 특정 스레드의 작업이 완료되기까지 기다릴 필요가 있는 경우 join이 필요하다.
* 참고) this
  *  this는 호출된 인스턴스 메서드가 소속된 객체를 가리키는 참조이며, 이것이 스택 프레임 내부에 저장되어 있다.
* join()
  * 스레드가 종료될 때 까지 대기
  * ex) thread1 종료될 때까지 대기
    ```java
    thread1.join();
    ```
  * join을 호출하는 스레드는 대상 스레드가 TERMINATED 상태가 될 때 까지 대기(WAITING)한다. 대상 스레드가 TERMINATED 상태가 되면 호출 스레드는 다시 RUNNABLE 상태가 되면서 다음 코드를 수행한다.
  * join의 단점은 다른 스레드가 완료될 때 까지 무기한 기다려야 한다.
* join(ms)
  * 쉽게 말해 제한 시간이 있는 join이다.
  * join(ms)를 호출하는 스레드는 TIMED_WAITING 상태가 된다.
